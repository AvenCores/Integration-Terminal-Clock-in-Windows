...
if platform == "win32":
    root = Tk()
    root.title('Terminal clock install')
    ...
    def installupgr():
        system("del /Q clock.py")
        f=open(r'clock.py', "wb")
        ufr = get("https://pastebin.com/raw/eAAh5ys8")
        f.write(ufr.content)
        f.close()

        f=open(r'install.bat', "wb")
        ufr = get("https://pastebin.com/raw/9jaYfHkY")
        f.write(ufr.content)
        f.close()
        system('install.bat')
        system('del /Q install.bat')
        system('del /Q clock.py')
        messagebox.showinfo(title="Установлено!", message='Terminal Clock был успешно установлен.')

    def remupgr():
        system('del /Q %USERPROFILE%\AppData\Local\Programs\Python\Python311\Scripts\clock.py')
        messagebox.showinfo(title="Удалено!", message='Terminal Clock был успешно удален.')
    ...
    def omyprog():
        messagebox.showinfo(title="О программе", message=f"""HZF Terminal Clock - это удобные часы, которые можно использовать прямо из Terminal.
